...
def formatBytesCount(size_bytes):
   if size_bytes == 0:
       return "0B"
...
class Toast:
    ...
    def draw(self,surface):
        ...
        if self.isShowing:
            ...
            surface.blit(self.font.render(text, True, self. text_color, self.bg_color),(surface.get_width()/2 - txtSize[0]/2, surface.get_height()/2 - txtSize[1]/2))        
class Button:
...
class ImageQualityModifier:
    def __init__(self) -> None:
        ...
        self.pad = 20
        self.dataValueDic = {"Original Image Resolution":self.imageOriginalResolution, "New Image Resolution":self.imageRenderSize, "Save path:":self.modifiedImagePath, "Size":0}
    ...
    def update(self):
        ...
        if self.resolution_slider.containsPoint(mouse_pos[0],mouse_pos[1]) or self.isDraggingOnSlider:
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                if self.isDraggingOnSlider and self.imgSurface != None:
                    ...
                    self.imgSurface = transform.scale(self.originalImageSurface,self.newImageResolution)
                    bytes_arr = io.BytesIO()
                    image.save(self.imgSurface, bytes_arr, self.extension)
                    self.dataValueDic["Size"] = (formatBytesCount(len(bytes_arr.getvalue())))
    ...
    def saveImage(self):
        try:
            if self.originalImageSurface != None and self.modifiedImagePath != None:
                ...
                image.save(transform.scale(self.originalImageSurface,self.newImageResolution),self.modifiedImagePath,self.extension)
    ...
    def render(self):
        ...
        if self.imgSurface != None:
            if self.imageRenderSize[0] != self.imgSurface.get_width() or self.imageRenderSize[1] != self.imgSurface.get_height():
                ...
                self.imgSurface = transform.scale(self.originalImageSurface,self.imageRenderSize)
    ...
    def loadImage(self,path):
        try:
            if isValidImagePath(path):
                ...
                self.extension =  os.path.splitext(self.modifiedImagePath)[1].lower()
                ...
                self.dataValueDic["New Image Resolution"] = str(self.newImageResolution[0]) + "x" + str(self.newImageResolution[1])       
                bytes_arr = io.BytesIO()
                image.save(self.originalImageSurface, bytes_arr, self.extension)
                self.dataValueDic["Size"] = (formatBytesCount(len(bytes_arr.getvalue())))
